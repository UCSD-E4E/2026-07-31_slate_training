"""The board plane — the quantity the laser calibration actually consumes.

`perform_laser_calibration_activity._laser_point_in_camera_space` uses exactly
two things from the fitted pose:
...
* **`upside_down` provably cannot affect the calibration**: a 180-degree
  in-plane flip yields an identical plane. That is the mathematical reason the
  field is read by nothing downstream.
...
(`reference_points / dpi -> inches -> metres`) remains essential. A homography
alone would recover the plane only up to scale.
"""
...
class Plane:
    ...
    negating both describes the identical plane, which `plane_difference`
    accounts for.
    """
...
def plane_from_pose(rotation: np.ndarray, translation: np.ndarray) -> Plane:
    ...
    The board is the z=0 plane in body coordinates, so its camera-space normal
    is the third column of `rotation` and `translation` is a point on it.
    """
...
def plane_difference(a: Plane, b: Plane) -> Tuple[float, float]:
    ...
    This is the parameterization-independent accuracy metric: it is exactly
    the part of a predicted labeling that reaches the calibration, and it is
    blind to the in-plane freedoms the calibration ignores.

    Normal sign is normalized first, since `n, d` and `-n, -d` are one plane.
    """
...
def laser_point_on_plane(
    ...
    Returns the 3-D hit in camera space, or None when the ray is parallel to
    the plane (or the projection degenerates) — the caller should drop that
    observation rather than emit a NaN, matching stage 13's behaviour.
    """
