...
from slate_training.mask import MASK_MEAN, MASK_STD, BoardMasker, preprocess
...
class TestImportableWithoutTorch:
    def test_module_imports_without_touching_torch(self):
        ...
        import slate_training.mask as m
        assert m.BoardMasker is not None

    def test_predict_slate_does_not_import_mask(self):
        # predict_slate must work with board_mask=None and no torch present.
        import slate_training.predict as p
        assert "slate_training.mask" not in getattr(p, "__dict__", {})
